...
plot(z[,c(2,3)], xlab = colnames(z)[2], ylab = colnames(z)[3], main = "Zebrafish RNA-seq")
plot(sc_z[,c(1,4)], xlab = colnames(sc_z)[1], ylab = colnames(sc_z)[4], main = "Pooled single-cell RNA-seq")
```
...
We plot the data colored by their maximum *a posterior* class labels. We can see that **modeling approach 3** appears the most robust, as approach 1 is not flexible enough, and approach 2 encounters identifiability issues, resulting in an unwanted merging of clusters.

```{r, cache = TRUE, fig.width = 10.5, fig.height = 7, warning = FALSE, message = FALSE}
par(mfrow = c(2,3))

fit_no_bound <- CLIMB::get_pairwise_fits(z[,c(2,3)], nlambda = 1, parallel = FALSE, bound = 0)
fit_no_bound_flex <- CLIMB::get_pairwise_fits(z[,c(2,3)], nlambda = 1, parallel = FALSE, bound = 0, flex_mu = TRUE)
fit_bound_flex <- CLIMB::get_pairwise_fits(z[,c(2,3)], nlambda = 1, parallel = FALSE, bound = qnorm(.8), flex_mu = TRUE)

sc_fit_no_bound <- CLIMB::get_pairwise_fits(sc_z[,c(1,4)], nlambda = 1, parallel = FALSE, bound = 0)
sc_fit_no_bound_flex <- CLIMB::get_pairwise_fits(sc_z[,c(1,4)], nlambda = 1, parallel = FALSE, bound = 0, flex_mu = TRUE)
sc_fit_bound_flex <- CLIMB::get_pairwise_fits(sc_z[,c(1,4)], nlambda = 1, parallel = FALSE, bound = qnorm(.8), flex_mu = TRUE)

plot(z[,c(2,3)], col = fit_no_bound[[1]]$cluster, main = "Approach 1")
plot(z[,c(2,3)], col = fit_no_bound_flex[[1]]$cluster, main = "Approach 2")
plot(z[,c(2,3)], col = fit_bound_flex[[1]]$cluster, main = "Approach 3")

plot(sc_z[,c(1,4)], col = sc_fit_no_bound[[1]]$cluster, main = "Approach 1")
plot(sc_z[,c(1,4)], col = sc_fit_no_bound_flex[[1]]$cluster, main = "Approach 2")
plot(sc_z[,c(1,4)], col = sc_fit_bound_flex[[1]]$cluster, main = "Approach 3")
```
