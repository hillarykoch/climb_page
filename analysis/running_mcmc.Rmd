...
### <i><b><span style="color:salmon">---This is likely the most time-consuming step of LIEB analysis---</span></b></i>
...
You can start an mcmc with the function `run_mcmc()`. This function executes everything at the default settings in the LIEB methodology. The user needs to provide 4 arguments:
...
2. `acceptane_rate_chain`: an $M\times\text{nstep}$ matrix of the acceptance rates for each cluster covariance. The proposals for each cluster are adaptively tuned such that the acceptance rates converge to about 0.3
